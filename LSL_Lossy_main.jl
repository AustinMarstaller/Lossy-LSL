...
reference_solutions_per_frequency = Matrix{Complex}(undef, length(x), length(λ))
for j = 1:length(λ)
    # For every λⱼ ∈ λ::Vector, generate the synthetic data via the Finite Difference scheme
    # Each column of solutions_per_frequency is a numerical solution for a particular λⱼ
    solutions_per_frequency[:,j]           = forward_solver(x,h,α,p,λ[j])
    reference_solutions_per_frequency[:,j] = forward_solver(x,h,α,p_reference,λ[j])
end
## ROM 
