...
function [v,C,D] = Lossy_synthetic_data(x,h,alpha,p,lambda)
   omega  = sqrt(-lambda);
   ...
   v = (-C + 1i*omega*D - lambda*eye(length(x)) )\f;

   figure
   hold on 
   plot(x,abs(exact_solution).^2)
   plot(x,abs(v).^2,'--')
   title("Zero potential finite-difference scheme for \lambda="+lambda)
   xlabel("GRID"),
   ylabel('$|v|^{2}$','Interpreter','latex')
end
